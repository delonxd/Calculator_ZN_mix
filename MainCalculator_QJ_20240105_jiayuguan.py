...
def main_cal(root, path2, path3):
    ...
    df_input = config_input_20240105_jiayuguan()
    ...
    # 获取表头
    head_list = config_headlist_20240105_jiayuguan()
    ...
    for temp_temp in range(num_len):
        ...
        # 配置数据
        config_row_data_20240105_jiayuguan(df_input_row, para, data)
        ...
        md = PreModel_20240105_QJ_jiayuguan(parameter=para)
        ...
        for posi_zhu in posi_list:
            ...
            MainLog.add_log_accurate('分路位置 --> %sm' % posi_zhu)
            ...
            v_rcv_1 = md.lg['线路3']['地面']['区段1']['左调谐单元']['1接收器']['U'].value_c
            v_rcv_2 = md.lg['线路3']['地面']['区段2']['左调谐单元']['1接收器']['U'].value_c
            ...
            data2excel.add_data(sheet_name="前方钢轨电流", data1=i_trk_zhu)
            data2excel.add_data(sheet_name="后方区段接收端轨入电压", data1=v_rcv_1)
            data2excel.add_data(sheet_name="前方区段接收端轨入电压", data1=v_rcv_2)
...
if __name__ == '__main__':
    sub_name = '20240105_嘉峪关分路不良'
    timestamp = time.strftime("%Y%m%d%H%M%S", time.localtime())
    main_cal(sub_name,
             '%s\\仿真输出_嘉峪关分路不良_%s.xlsx' % (sub_name, timestamp),
             os.getcwd())
