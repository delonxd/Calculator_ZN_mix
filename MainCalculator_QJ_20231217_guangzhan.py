...
def main_cal(_, path2, path3):
    ...
    # 参数输入
    df_input = config_input_20231217_guangzhan()
    ...
    # 获取表头
    head_list = config_headlist_20231217_guangzhan()
    ...
    for temp_temp in range(num_len):
        ...
        # 配置数据
        config_row_data_20231217_guangzhan(df_input_row, para, data)

        if data_flag != '' and data_flag != data['主被串线路']:
            write2excel_main(path2, data2excel, excel_data, head_list)
            excel_data = []
            data2excel = SheetDataGroup(sheet_names=[])
        ...
        md = PreModel_QJ_20231217_guangzhan(parameter=para)
...
def write2excel_main(dir_path, data2excel, excel_data, head_list):
    ...
    str3 = df_data.loc[0, '主被串线路']
    str4 = df_data.loc[0, '南广区段']
    str5 = df_data.loc[0, '广湛频率']
    str6 = df_data.loc[0, '分路模式']

    str3456 = '%s_南广%s_广湛%sHz_%s' % (str3, str4, str5, str6)
    filepath = '%s\\仿真输出_广湛六线并行_%s_%s.xlsx' % (dir_path, str2, str3456)
...
if __name__ == '__main__':
    ...
    main_cal(None,
             '%s\\仿真输出_广湛六线并行_%s' % (sub_name, timestamp),
             os.getcwd())
