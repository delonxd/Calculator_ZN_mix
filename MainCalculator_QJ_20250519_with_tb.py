...
from src.Config_QJ_20250519_with_tb import config_input_20250519_with_tb
from src.Config_QJ_20250519_with_tb import config_headlist_20250519_with_tb
from src.Config_QJ_20250519_with_tb import config_row_data_20250519_with_tb
from src.Config_QJ_20250519_with_tb import PreModel_QJ_20250519_with_tb
...
def main_cal(_, path2, path3, list_type, list_length, list_freq):
    ...
    df_input = config_input_20250519_with_tb(list_type, list_length, list_freq)
    ...
    # 获取表头
    head_list = config_headlist_20250519_with_tb()
    ...
    for temp_temp in range(num_len):
        ...
        # 配置数据
        config_row_data_20250519_with_tb(df_input_row, para, data)
        ...
        md = PreModel_QJ_20250519_with_tb(parameter=para)
...
def cycle_cal():
    ...
    sub_name = '20250519_区间换装TB'
    timestamp = time.strftime("%Y%m%d%H%M%S", time.localtime())

    dir_path = '%s\\仿真输出_区间换装TB_%s' % (sub_name, timestamp)
    ...
    for val_type in list_type:
        for val_length in list_length:
            for val_freq in list_freq:
                ...
                file_name = '区间换装TB_%s对%s_%sm_主串%sHz_被串%sHz.xlsx' % values_tmp
