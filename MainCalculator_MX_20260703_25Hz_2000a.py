...
from src.Config_ZN_20260703_25Hz_2000a_mix import config_input_20260703_25hz_2000a_mix
from src.Config_ZN_20260703_25Hz_2000a_mix import config_headlist_20260703_25hz_2000a_mix
from src.Config_ZN_20260703_25Hz_2000a_mix import config_row_data_20260703_25hz_2000a_mix
from src.Config_ZN_20260703_25Hz_2000a_mix import PreModel_20260703_25hz_2000a_mix
...
def main_cal(output_path, work_path):
    ...
    for temp_temp in range(num_len):
        ...
        for posi_bei in posi_list:
            ...
            if data['被串区段类型'] == '2000A区间':
                index_bei = data['被串区段索引']
                sec_name = f"区段{int(index_bei+1)}"

                if data['被串方向'] == '右发':
                    v_rcv_bei = md.lg['线路4']['地面'][sec_name]['左调谐单元']['1接收器']['U'].value_c
                else:
                    v_rcv_bei = md.lg['线路4']['地面'][sec_name]['右调谐单元']['1接收器']['U'].value_c
            else:
                v_rcv_bei = 'None'
        ...
        print(data2excel.data_dict["被串轨入电压"][-1])
...
if __name__ == '__main__':
    ...
    file = '20260703_2000A电码化混合'
    name = '20260703_2000A电码化混合'
    timestamp = time.strftime("%Y%m%d%H%M%S", time.localtime())

    path1 = f'{file}\\仿真输出_{name}_{timestamp}.xlsx'
    path2 = os.getcwd()
