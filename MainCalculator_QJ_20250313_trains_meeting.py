...
from src.Config_QJ_20250313_trains_meeting import config_input_20250313_trains_meeting
from src.Config_QJ_20250313_trains_meeting import config_headlist_20250313_trains_meeting
from src.Config_QJ_20250313_trains_meeting import config_row_data_20250313_trains_meeting
from src.Config_QJ_20250313_trains_meeting import PreModel_QJ_20250313_trains_meeting
...
def main_cal(_, path2, path3, list_type, list_length, list_freq):
    ...
    df_input = config_input_20250313_trains_meeting(list_type, list_length, list_freq)
    ...
    # 获取表头
    head_list = config_headlist_20250313_trains_meeting()
    ...
    for temp_temp in range(num_len):
        ...
        show_scene = df_input_row.tolist()
        show_scene = show_scene[1:]
        MainLog.add_log_accurate('scene %s %s ...' % (counter, show_scene))

        # 配置数据
        config_row_data_20250313_trains_meeting(df_input_row, para, data)
        ...
        md = PreModel_QJ_20250313_trains_meeting(parameter=para)
        ...
        # posi_list = [posi_list[0], posi_list[-1]]
        for posi_bei in posi_list:
            ...
            if len(tmp_posi) == 100 or posi_bei == posi_list[-1]:
...
def cycle_cal():
    ...
    #     # 400,
    # ]
    # list_length = list(range(400, 801, 100))
    list_length = [800]
    ...
    sub_name = '20250313_区间列车会车'
    timestamp = time.strftime("%Y%m%d%H%M%S", time.localtime())

    dir_path = '%s\\仿真输出_区间列车会车遍历_%s' % (sub_name, timestamp)
    ...
    for val_type in list_type:
        for val_length in list_length:
            for val_freq in list_freq:
                values_tmp = (val_type[0], val_type[1], val_length, val_freq[0], val_freq[1])
                file_name = '区间会车_%s对%s_%sm_主串%sHz_被串%sHz.xlsx' % values_tmp
