...
# 配置输入
def config_input_20241218_qinghezhan():
...
# 配置表头
def config_headlist_20241218_qinghezhan():
    head_list = [
        '序号',
        '备注',
        '分路模式',
...
def config_c_pack_20241218_qinghezhan(freq_list, length_list, c_value_src):
...
# 配置行数据
def config_row_data_20241218_qinghezhan(df_input, para, data):
    # 序号
    ...
    # 区段名
    data['主串区段'] = para['主串区段'] = '[S1LQG]'
    data['被串区段'] = para['被串区段'] = '[0281G, 0293BG, 0293AG]'

    # 区段长度
    para['主串区段长度'] = [1174]
    para['被串区段长度'] = [1206, 797, 803]

    para['主串绝缘节长度'] = [29, 0]
    para['被串绝缘节长度'] = [29, 29, 29, 0]

    data['主串区段长度(m)'] = para['主串区段长度']
    data['被串区段长度(m)'] = para['被串区段长度']

    # 相对位置
    data['被串相对位置(m)'] = offset = -1632

    para['offset_zhu'] = 0
    para['offset_bei'] = -1632

    # 耦合系数
    data['耦合系数(μH/km)'] = para['耦合系数'] = 20

    # 区段频率
    para['freq_主'] = freq1 = data['主串频率(Hz)'] = 2600
    para['freq_被'] = freq2 = data['被串频率(Hz)'] = 1700
    freq = freq1
    data['freq'] = para['freq'] = Freq(freq1)
    para['主串频率列表'] = [Freq(2600)]
    para['被串频率列表'] = [Freq(2300), Freq(1700), Freq(2300)]

    data['主串区段类型'] = '高铁区间'
    data['被串区段类型'] = '高铁区间'

    # 电容配置
    c_pack_zhu = config_c_pack_20241218_qinghezhan(para['主串频率列表'], para['主串区段长度'], [25])
    c_pack_bei = config_c_pack_20241218_qinghezhan(para['被串频率列表'], para['被串区段长度'], [25, 25, 25])
    ...
    para['主串阻抗列表'] = c_pack_zhu['电容阻抗列表']
    para['被串阻抗列表'] = c_pack_bei['电容阻抗列表']
    ...
    data['分路起点'] = offset + 1206 - 14.5
    data['分路终点'] = offset + 1206 + 797 + 14.5


class PreModel_20241218_QJ_qinghezhan(PreModel):
