...
# 配置表头
def config_headlist_20231225_add_capacitor():
    head_list = [
        '序号',
        '备注',
        '主串区段', '被串区段',
...
# 配置行数据
def config_row_data_20231225_add_capacitor(df_input, para, data):
    # 序号
...
class PreModel_20231225_ZN_ypmc_v04_add_capacitor(PreModel):
    ...
    def change_para_el(self):
        ...
        for ele in self.section_group3['区段1'].element.values():
            if isinstance(ele, ZPW2000A_YPMC_add_cap):
