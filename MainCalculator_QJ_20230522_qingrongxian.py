...
def main_cal(path1, path2, path3):
    ...
    for temp_temp in range(num_len):
        ...
        md = PreModel_20230522_qingrongxian(parameter=para)
...
if __name__ == '__main__':
    # sub_name = '20230522_青荣线'
    sub_name = '20230601_区间普铁高铁对比'
    timestamp = time.strftime("%Y%m%d%H%M%S", time.localtime())
    main_cal('邻线干扰参数输入_%s.xlsx' % sub_name,
             '仿真输出_%s_%s.xlsx' % (sub_name, timestamp),
             os.getcwd())
